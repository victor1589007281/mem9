...
func (r *MemoryRepo) GetByID(ctx context.Context, id, agentID string) (*domain.Memory, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT `+allColumns+` FROM memories WHERE id = ? AND agent_id = ? AND state = 'active'`, id, agentID,
	)
...
func (r *MemoryRepo) UpdateOptimistic(ctx context.Context, m *domain.Memory, expectedVersion int) error {
	...
	query := `UPDATE memories SET content = ?, tags = ?, metadata = ?, embedding = ?, version = version + 1, updated_by = ?, updated_at = datetime('now') WHERE id = ? AND agent_id = ?`
...
func (r *MemoryRepo) SoftDelete(ctx context.Context, id, agentID string) error {
	...
	err = tx.QueryRowContext(ctx, `SELECT state FROM memories WHERE id = ? AND agent_id = ?`, id, agentID).Scan(&state)
	...
	_, err = tx.ExecContext(ctx, `UPDATE memories SET state = 'deleted', updated_at = datetime('now') WHERE id = ? AND agent_id = ?`, id, agentID)
	if err != nil {
...
func (r *MemoryRepo) ArchiveMemory(ctx context.Context, id, agentID, supersededBy string) error {
	result, err := r.db.ExecContext(ctx,
		`UPDATE memories SET state = 'archived', superseded_by = ?, updated_at = datetime('now')
		 WHERE id = ? AND agent_id = ? AND state = 'active'`,
...
func (r *MemoryRepo) SetState(ctx context.Context, id, agentID string, state domain.MemoryState) error {
	result, err := r.db.ExecContext(ctx,
		`UPDATE memories SET state = ?, updated_at = datetime('now') WHERE id = ? AND agent_id = ? AND state = 'active'`,
...
func (r *MemoryRepo) Count(ctx context.Context, agentID string) (int, error) {
	var count int
	err := r.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM memories WHERE agent_id = ? AND state = 'active'`, agentID).Scan(&count)
	if err != nil {
...
func (r *MemoryRepo) ListBootstrap(ctx context.Context, agentID string, limit int) ([]domain.Memory, error) {
	...
	rows, err := r.db.QueryContext(ctx,
		`SELECT `+allColumns+` FROM memories WHERE agent_id = ? AND state = 'active' ORDER BY updated_at DESC LIMIT ?`,
		agentID, limit,
	)
	if err != nil {
